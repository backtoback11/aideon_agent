...
async def step3_fill_recipient_and_sender(page: Page, invoice) -> None:
    ...
    ставим галочку согласия, жмём 'Продолжить' → дальше должна быть капча.
    """
    ...
    try:
        ...
        await _fill_by_label_or_name(
            ...
            name_attr=None,  # rely on label, name мог поменяться
        ...
        await _fill_by_label_or_name(
            ...
            name_attr=None,
            value=getattr(invoice, "sender_passport_issue_date", None),
        )
        ...
        await _fill_by_label_or_name(
            ...
            name_attr="birthDate",
            value=invoice.sender_birth_date,
        )
